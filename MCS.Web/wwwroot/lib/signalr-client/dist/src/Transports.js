    ...
    connect(url, requestedTransferMode) {
        return new Promise((resolve, reject) => {
            ...
            webSocket.onmessage = (message) => {
                this.logger.log(ILogger_1.LogLevel.Information, `(WebSockets transport) data received: ${message.data}`);
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                eventSource.onmessage = (e) => {
                    if (this.onDataReceived) {
                        try {
                            this.logger.log(ILogger_1.LogLevel.Information, `(SSE transport) data received: ${e.data}`);
                ...
                if (this.onDataReceived) {
                    try {
                        let response = transferMode === 1 /* Text */
                            ...
                            : pollXhr.response;
                        if (response) {
                            this.logger.log(ILogger_1.LogLevel.Information, `(LongPolling transport) data received: ${response}`);
